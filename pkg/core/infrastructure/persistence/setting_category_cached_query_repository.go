...
import (
	...
	appsetting "github.com/lwmacct/260101-go-pkg-ddd/pkg/core/application/setting"
	"github.com/lwmacct/260101-go-pkg-ddd/pkg/core/domain/setting"
)
...
func (r *cachedSettingCategoryQueryRepository) FindByID(ctx context.Context, id uint) (*setting.SettingCategory, error) {
	...
	if err != nil {
		slog.Warn("cache get all categories failed, fallback to db", "err", err)
	}
	for _, c := range all {
		if c.ID == id {
			return c, nil
...
func (r *cachedSettingCategoryQueryRepository) FindAll(ctx context.Context) ([]*setting.SettingCategory, error) {
	...
	if len(result) > 0 {
		if err := r.cache.SetAllCategories(ctx, result); err != nil {
			slog.Warn("cache set all categories failed", "count", len(result), "err", err)
		}
	}
